        ...
        <View style={styles.container}>
            ...
            <View style={styles.footer}>
                ...
                <View style={styles.containerIcons}>
                    <TouchableOpacity style={{paddingRight:10}} /* onPress={trocaCaveira} */ onPress={like}>
                        <Image source={ liked ? caveiralike : caveira } style={{height:30, width:30}}></Image>
                    </TouchableOpacity>
                    <TouchableOpacity style={{paddingRight:10}} onPress={() => navigation.navigate('AddComment', { post, autor })}>
                        <Image source={Comentario} style={{height:30, width:30}}></Image>
                    </TouchableOpacity>
                    {!repost && <TouchableOpacity style={{paddingRight:10}} onPress={() => navigation.navigate('Repost', { post, autor })}>
            ...
            >
                <Text style={[styles.legenda]}>
                    <Text style={{ fontWeight: 'bold' }}>{autor?.nome}</Text> {post.descricao}
                </Text>

                { !repost && <Text style={styles.titleComments}>Ver todos {numComments} comentários</Text> }

                {
                    !repost && comments.map((item, i) => (
                        <Text key={item.id} style={styles.legenda}>
                            <Text style={{ fontWeight: 'bold' }}>{item.nomeAutor}</Text> {item.comment}
                        </Text>
                    ))
                }
            </TouchableOpacity>
