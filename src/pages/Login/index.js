  ...
  const loginWithFacebok = async () => {
    try {
      ...
      const {
        ...
        token,
        ...params
      } = await FacebookAuthentication.logInWithReadPermissionsAsync();
  ...
  const loginWithGoogle = async () => {
    try {
      const { type, accessToken, user } = await GoogleAuthentication.logInAsync(
        {
          clientId:
            "993866057606-c8ir7l07ri24lbg5di834g28479ovj15.apps.googleusercontent.com",
        }
      );
      ...
      >
        ...
        >
          <Image
            source={Logo}
            style={{
              height: Image.resolveAssetSource(Logo).height * 0.8,
              width: Image.resolveAssetSource(Logo).width * 0.8,
              marginTop: 10,
            }}
          />
        </View>
